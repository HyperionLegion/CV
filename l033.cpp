...
void part1(){ //brute forcing
    ...
    Point min1;
    Point min2;
    for(int i=0;i<100000; i++){ // change number of points here
...
void part4(){
    ...
    cout << std::setprecision(26) << "Part 4: "<< "("<<dist.getPoint1().getX()<<","<<dist.getPoint1().getY()<<") , " << "("<<dist.getPoint2().getX()<<","<<dist.getPoint2().getY()<<")\nDistance: " <<dist.getPoint1().distance(dist.getPoint2())<<"\nTime: " << std::setprecision(1)<<duration_cast<microseconds>(stop - start).count() << " microseconds\n";
}
int main(){
    //part1();
    //part2();
